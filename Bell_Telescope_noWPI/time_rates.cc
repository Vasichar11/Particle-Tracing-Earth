...
{  
    ...
    if(std::isnan(wtau_sq*m_e/kz))  //for no interaction to be valid.
    {
        lrk=-(1/(m_e*gama))*((pper_tmp*pper_tmp)/(2*w_h))*dwh_ds;  //((wtau_sq*m_e)/kz)*sin(eta_tmp)=0.
    }
    else
    {
        //Borntik thesis 2.24a
        lrk=((wtau_sq*m_e)/kz)*sin(eta_tmp)-(1/(m_e*gama))*((pper_tmp*pper_tmp)/(2*w_h))*dwh_ds;
    }    
...
real lamda_rk(real ppar_tmp,real lamda_tmp,real m_e, real gama)
{
    ...
    real ork=ppar_tmp/(gama*m_e*Constants::L_shell*Constants::Re*sqrt(1+3*sin(lamda_tmp)*sin(lamda_tmp))* cos(lamda_tmp));
    return ork;
}
...
real alpha_rk(real pper_tmp,real alpha_tmp,real eta_tmp,real m_e, real kz, real w_h, real w, real dwh_ds, real gama, real wtau_sq)
{
    ...
    if(std::isnan(m_e*wtau_sq/(kz*pper_tmp))) //for no interaction to be valid.
    {
        prk = ((1/(m_e*gama))*(pper_tmp/(2*w_h)))*dwh_ds;
    }
    else
    {   
        //Bortnik thesis 2.26
        prk=(-((m_e*wtau_sq/(kz*pper_tmp))*(1+((cos(alpha_tmp)*cos(alpha_tmp))/(Constants::m_res*(w_h/w)-1))))*sin(eta_tmp)+
