...
namespace Simulation
{
    ...
    const real zm = (Universal::Re + hm)/(L_shell*Universal::Re);
...
namespace Satellite
{
    ...
    const real telescope_latitude = 0;  //Position of the satellite
}
...
namespace Distribution
{
    ...
    const real eta0=eta0_deg*Universal::D2R;
    const real aeq0_deg=69.844;                     // If single values are used.
    const real aeq0=aeq0_deg*Universal::D2R;
    const real latitude0_deg=-9;                       // If single values are used.
    const real latitude0=latitude0_deg*Universal::D2R;
    ...
    // Latitude dstr
    const real latitude_start_d  =-90;    		  			 				
    const real latitude_end_d    = 90;  	
    // Eta dstr
    ...
    // Normal dstr parameters
    const real mean_aeq       = 90;  // Mean of the normal dstr
    const real mean_latitude     = 0; 
    ...
    const real max_stdev_latitude= 20;  // Standard deviation when latitude domain is the biggest. Is propotional to latitude range. 
    const real latitude_domain_step = 0.001; // Latitude domain range precision
    // Even dstr parameters
    const real aeq_steps= 10; 
    const real latitude_steps= 10;
    const real eta_steps= 10;
    const real Ekin_steps= 10;
