...
void li_wpi(const int64_t Nsteps_wpi, const int p, Particles &particle, Telescope &ODPT, Ray &ray)
{
//---------------------------------------------------- ASSIGN OBJECT VALUES ----------------------------------------------------//
    ...
    real latitude = particle.latitude_end;
    real ppar = particle.ppar_end; 
    real pper = particle.pper_end; 
    real alpha = particle.alpha_end; 
    real aeq = particle.aeq_end; 
    real time = particle.time_end;
    real eta = particle.eta_end; 
    real Ekin = particle.Ekin_end; 
    real deta_dt;
    //real zeta = particle.zeta_end; 
    //real upar = particle.upar_end; 
    //real uper = particle.uper_end;
...
//----------------------------------------------------- WPI -----------------------------------------------------------//
    for(int i=0; i<Nsteps_wpi; i++) {
        ...
        min_lat=*min_element(ray.lat_int.cbegin() + i, ray.lat_int.cbegin() + Simulation::puls_dur + i);  //Minimum lat of wave(in pulse duration).
        max_lat=*max_element(ray.lat_int.cbegin() + i, ray.lat_int.cbegin() + Simulation::puls_dur + i);  //Max lat of wave.
        //std::cout<<"\n" << latitude*Universal::R2D << " " << min_lat << "" << max_lat << " "<< ray.lat_int.at(i) << " " << ray.lat_int.back(); 
        //std::cout <<"\n\np_mag "<<p_mag<<"\ndwh_ds "<< dwh_ds<<"\nkz "<< kz  <<"\nFpar "<< Fpar <<"\nFper "<< Fper <<"\nFtheta "<< Ftheta <<"\n";  

        f_always(p_mag, gama, w_h, dwh_ds, latitude, ppar, pper); 
        kz = Ftheta = Fpar = Fper = q1 = 0; 
        index = is_in_packet(min_lat, max_lat, latitude, i, ray.lat_int);  //is_in_packet returns "if and where" WPI happens. 
        if(index>=0) { //Do only if there's WPI
            f_packet(Fpar, Fper, Ftheta, q1, kz, pper, ppar, eta, aeq, alpha, gama, w_h, p_mag, ray.kx_ray[index], ray.kz_ray[index], ray.kappa_ray[index], ray.Bw_ray[index], ray.Bzw[index], ray.Ezw[index], ray.w1[index], ray.w2[index], ray.R1[index], ray.R2[index]);
        }
        ...
        index = is_in_packet(min_lat, max_lat, latitude+0.5*Simulation::h*o1, i, ray.lat_int);
        if(index>=0) { //Do only if there's WPI
            f_packet(Fpar, Fper, Ftheta, q2, kz, pper+0.5*Simulation::h*m1, ppar+0.5*Simulation::h*l1, eta+0.5*Simulation::h*n1, aeq+0.5*Simulation::h*q1, alpha+0.5*Simulation::h*p1, gama, w_h, p_mag, ray.kx_ray[index], ray.kz_ray[index], ray.kappa_ray[index], ray.Bw_ray[index], ray.Bzw[index], ray.Ezw[index], ray.w1[index], ray.w2[index], ray.R1[index], ray.R2[index]);
        }
        ...
        index = is_in_packet(min_lat, max_lat, latitude+0.5*Simulation::h*o2, i, ray.lat_int);
        if(index>=0) { //Do only if there's WPI
            f_packet(Fpar, Fper, Ftheta, q3, kz, pper+0.5*Simulation::h*m2, ppar+0.5*Simulation::h*l2, eta+0.5*Simulation::h*n2, aeq+0.5*Simulation::h*q2, alpha+0.5*Simulation::h*p2, gama, w_h, p_mag, ray.kx_ray[index], ray.kz_ray[index], ray.kappa_ray[index], ray.Bw_ray[index], ray.Bzw[index], ray.Ezw[index], ray.w1[index], ray.w2[index], ray.R1[index], ray.R2[index]);
        }
        ...
        index = is_in_packet(min_lat, max_lat, latitude+Simulation::h*o3, i, ray.lat_int);
        if(index>=0) { //Do only if there's WPI
            f_packet(Fpar, Fper, Ftheta, q4, kz, pper+Simulation::h*m3, ppar+Simulation::h*l3, eta+Simulation::h*n3, aeq+Simulation::h*q3, alpha+Simulation::h*p3, gama, w_h, p_mag, ray.kx_ray[index], ray.kz_ray[index], ray.kappa_ray[index], ray.Bw_ray[index], ray.Bzw[index], ray.Ezw[index], ray.w1[index], ray.w2[index], ray.R1[index], ray.R2[index]);
        }
